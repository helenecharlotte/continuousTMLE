...
est.fun <- function(dt, censoring=TRUE, intervention.A=c(1, 1), stochastic.A=FALSE,
                    ...
                    test=FALSE, rescue=FALSE, extra=FALSE, estimate.Gstar=FALSE, 
    ...
    if (stochastic.A & length(intervention.A)==0) intervention.A <- 1
    ...
    if (smooth.initial) {
        ...
        if (misspecify.Q) {
            ...
            if (rescue) {
                #fit.Y <- glm(Y ~ L0 + A0*L.prev + L.prev + dN.A.prev,
                #             data=dt.long[keep==1], family=binomial())
                fit.Y <- glm(Y ~ L0 + A0*L.prev + A.prev*L.prev + dN.A.prev,
                             data=dt.long[keep==1], family=binomial())
            }
        ...
        if (stochastic.A & rescue & !estimate.Gstar) { #--- stochastic intervention that is estimated from the data;
            fit.A[, "int.A":=0.5*plogis(form.A(L0, L.prev, A.prev, 1))+
                        0.5*plogis(form.A(L0, L.prev, A.prev, 0))]
            fit.A[, "int.A":=int.A*(A==0) + (1-int.A)*(A==1)] 
        } else if (stochastic.A) { #--- stochastic intervention that is estimated from the data;
    ...
    for (k in 0:K) {
        ...
        if (smooth.initial) {
            if (k>1) {
                ...
                dt[, `:=`(k=k, L.prev=0, L=get(paste0("L", k)), A.prev=0,
            ...
            if (k>0) {
                ...
                if (length(intervention.A)>0 | stochastic.A) {
    ...
    dt.Z.list <- lapply(1:(length(var.order)-1), function(k) {
        ...
        if (substr(k.var, 1, 1) %in% c("L", "A") & numextract(k.var)>0) {
            ...
            if (numextract(k.var)>1 | ((substr(k.var, 1, 1)=="A") & !rescue)) {
    ...
    #-------------------------------------------------------------------------------------------#
    ## function for removing variables in dataset 
    #-------------------------------------------------------------------------------------------#
    ...
    target.fun <- function(dt, iter=0, targeting=1) {
        ...
        dt.tmle <- do.call("rbind", lapply(non.int.nodes, function(var) {
            ...
            } else if (stochastic.A & extra & substr(var, 1, 1)=="A") {
                tmp <- dt[, c(var, paste0("pred.", var), paste0("H.", var),
                              paste0("WA", numextract(var))), with=FALSE]
            } else {
                tmp <- dt[, c(var, paste0("pred.", var), paste0("H.", var),
    ...
    compute.eic <- function(dt, eic.name="eic", browse=FALSE, targeting=1) {
        ...
        for (var in non.int.nodes) {
            if (targeting==2 & substr(var, 1, 1)=="L") {
                ...
                dt[, tmp.eic:=tmp.eic+get(paste0("H.", var))*get(paste0("WA", numextract(var)))*
